...
mem_init_mp(void)
{
	...
	{
		boot_map_region(kern_pgdir,
				KSTACKTOP - (i + 1) * KSTKSIZE - i * KSTKGAP,
				KSTKSIZE,
...
page_init(void)
{
	...
	//calculate bound index in pages array
	int leftPAIndex = ((int)IOPHYSMEM) / PGSIZE;
	int rightPAIndex = ((int)(ROUNDUP(PADDR(nextfree),PGSIZE))) / PGSIZE;
	int MPENTRY = ((int)MPENTRY_PADDR) / PGSIZE;
	...
	for (i = 0; i < npages; i++) 
	{
		...
		if((i == 0) || (i == MPENTRY))
			continue;
		//< or <=? need reconsider
		if(i >= leftPAIndex && i < rightPAIndex)
			continue;
...
page_alloc(int alloc_flags)
{
	...
	struct Page * retPage = NULL;
	if(page_free_list == NULL)
		return NULL;
	if(alloc_flags & ALLOC_ZERO)
		memset(page2kva(page_free_list), 0, PGSIZE);
	retPage = page_free_list;
...
{
	pte_t * pt_entry = pgdir + PDX(va);
	pte_t * pt_kva = NULL;
	
	//check present bit. If exist
	if(*pt_entry & PTE_P)
	{
		pt_kva = KADDR(PTE_ADDR(*pt_entry));
	...
	{
		struct Page * new_pt_page = NULL;
		new_pt_page = page_alloc(1);
		if(new_pt_page != NULL)
		{
			memset(page2kva(new_pt_page), 0, PGSIZE);
			new_pt_page->pp_ref ++;

			*pt_entry = PADDR(page2kva(new_pt_page)) | PTE_U | PTE_W | PTE_P;
			pt_kva = KADDR(PTE_ADDR(*pt_entry));
			return pt_kva + PTX(va);				
		}
		else
...
{
	pte_t * pt_entry = NULL;
	struct Page * pp = page_lookup(pgdir, va, &pt_entry);
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	uintptr_t left = (uintptr_t)va;
	uintptr_t right = (uintptr_t)(va + len);
	uintptr_t curva;
	pte_t * pt_entry = NULL;
	perm = perm | PTE_P;	
	
	for(curva = left; curva < right; curva += PGSIZE)
	{
		if(curva >= ULIM)
		{
		...
		pt_entry = pgdir_walk(env->env_pgdir, (const void *)curva, 0);
		if((pt_entry == NULL) || ((*pt_entry & perm) != perm))
		{
			user_mem_check_addr = curva;
			return -E_FAULT;
		}
